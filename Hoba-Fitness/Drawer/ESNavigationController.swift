...
class ESNavigationController: UINavigationController, UIGestureRecognizerDelegate {
    ...
    fileprivate var rightWidth: CGFloat = 250
    fileprivate var rightRevealType: RevealType = .slideOver// .slideUnder
    ...
    // swipe access
    fileprivate var panLimitedAccess: Bool = true
    fileprivate var panAccessView: MenuType = .leftMenu
    fileprivate var leftPanAccessRange: CGFloat = 10
    fileprivate var rightPanAccessRange: CGFloat = 10
    ...
    func openMenu(_ menu: MenuType, animated:Bool, completion:(()->(Void))?){
    ...
    fileprivate func changeMenu(_ menu: MenuType, animated: Bool, percentage: CGFloat, completion:(()->(Void))?){
    ...
    fileprivate func animateLayoutChanges(_ menu: MenuType, percentage: CGFloat, speed: CGFloat, completion:(()->(Void))?){
...
extension ESNavigationController {
    ...
    fileprivate func getHorizontalConstraintForMenu(_ menu: MenuType) -> NSLayoutConstraint{
        //return NSLayoutConstraint(item: self.rightMenuViewController.view, attribute: .leading, relatedBy: .equal, toItem: self.view.window!, attribute: .trailing, multiplier: 1, constant:  0)
        //return (menu == .leftMenu) ? self.leftTrailingConstraint! : self.rightLeadingConstraint!
        return self.rightLeadingConstraint!
    }
}
...
extension ESNavigationController {
    ...
    @objc internal func panEventFired(_ gesureRecognizer: UIPanGestureRecognizer){
